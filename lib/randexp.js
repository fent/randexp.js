...
/**
 * expand sets to contain a list of available characters
 *
 * @param {Array} arr
 * @return {Array}
 */
function expand(arr) {
  var map = {};
  var output = [];
  function addCharElement(value) {
    if (map[value]) return;
    map[value] = true;
    output.push({type: ret.types.CHAR, value: value});
  }
  arr.forEach(function (element) {
    if (element.type === ret.types.CHAR) {
      addCharElement(element.value);
    } else if (element.type == ret.types.RANGE) {
      for (var i = element.from; i <= element.to; i++) addCharElement(i);
    } else if (element.type == ret.types.SET) {
      var expanded_set = expand(element.set);
      if(element.not) {
        var values = expanded_set.map(function (element) {
        ...
        expanded_set.forEach(function (element) {
            addCharElement(element.value);
        });
      }
    }
  ...
  switch (token.type) {
    ...
    case types.SET:
      ...
      // Otherwise, pick a random token in the class set.
        ...
        if (token.set.length) {
          var expanded_set = expand(token.set);
          return gen.call(this,
                 expanded_set[Math.floor(Math.random() * expanded_set.length)],
                 groups, not);
        } else {
          return '';
        }
