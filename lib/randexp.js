 ...
 * @return {Object}
 */
function _randSelect(arr) {
  if (arr instanceof DRange) {
    return arr.index(RandExp.randInt(0, arr.length - 1));
  }
  return arr[RandExp.randInt(0, arr.length - 1)];
}
...
function char(code, ignoreCase) {
  code = ignoreCase && _randBool() ? toOtherCase(code) : code;
...
function expand(token) {
  ...
  if (token.type === ret.types.SET) {
    ...
    if (token.not) {
      return RandExp.defaultRange.clone().subtract(drange);
    } else {
      return drange;
    }
  }
...
function gen(token, groups) {
  ...
  switch (token.type) {
    ...
    case types.GROUP:
      ...
      stack = token.options ? _randSelect(token.options) : token.stack;
    ...
    case types.SET:
      ...
      return char(_randSelect(expanded_set), this.ignoreCase);

    case types.RANGE:
      return char(RandExp.randInt(token.from, token.to), this.ignoreCase);


    case types.REPETITION:
      // Randomly generate number between min and max.
      n = RandExp.randInt(token.min,
...
RandExp.defaultRange = new DRange(32, 126);

// This allows custom PRNGs for seedable and/or cryptographic results
RandExp.randInt = _randInt;
