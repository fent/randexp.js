var ret = require('ret');
var RangeSet = require('./range_set');
var types = ret.types;
...
/**
 * expands a token to a RangeSet of characters which has a 
...
function expand(token) {
  if (token.type == ret.types.CHAR) return new RangeSet(token.value);
  if (token.type == ret.types.RANGE) return new RangeSet(token.from, token.to);
  if (token.type == ret.types.SET) {
    if (token.not) {
      var rs = new RangeSet(0, 65535);
      token.set.forEach(function (sub_token) {
        var expanded_sub_token = expand(sub_token);
        expanded_sub_token.ranges.forEach(function (range) {
          rs.subtract(range.low, range.high);
        });
      });
      return rs;
    } else {
...
function gen(token, groups, negate) {
  ...
  switch (token.type) {
    ...
    case types.SET:
      ...
      return char(expanded_set.index(Math.floor(Math.random() * expanded_set.length)), this.ignoreCase);
      break;
