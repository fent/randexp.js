...
function gen(token, groups, negate) {
  ...
  switch (token.type) {
    ...
    case types.SET:
      ...
      var random_value = expanded_set.index(Math.floor(Math.random() * expanded_set.length));
      return char(random_value, this.ignoreCase);
      break;
...
RandExp.setRandIntFunction = function(_randInt) {
  randInt = _randInt;
}
